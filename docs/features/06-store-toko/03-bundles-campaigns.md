...
Fitur **Bundles Campaigns** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Bundles Campaigns** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
| Item | Value |
|------|-------|
| UI routes/surfaces | `/toko, /dashboard/toko` |
...
| PATCH | `/api/store/admin/bundles/:id` | `server/routes/store.ts#L1192` | params: id | 404, 400, 500, 200/json |
| DELETE | `/api/store/admin/bundles/:id` | `server/routes/store.ts#L1243` | params: id | 404, 500, 200/json |
...
- Feature doc: `06-store-toko/03-bundles-campaigns.md`
- UI: `/toko, /dashboard/toko`
