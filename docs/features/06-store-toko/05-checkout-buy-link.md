...
Fitur **Checkout Buy Link** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Checkout Buy Link** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
| Item | Value |
|------|-------|
| UI routes/surfaces | `/toko/cart, /toko/:slug` |
...
|--------|----------|--------|----------------|-------------------|
| POST | `/api/store/checkout` | `server/routes/store.ts#L2422` | none observed in handler window | verify in handler |
| POST | `/api/store/direct-checkout` | `server/routes/store.ts#L2427` | none observed in handler window | 400 |
| POST | `/api/store/buy-link` | `server/routes/store.ts#L2439` | body: productId, qty | 400, 500, 200/json |

---

## Observed Request Shape

- `POST /api/store/buy-link` body fields observed: `productId, qty`
...
- Feature doc: `06-store-toko/05-checkout-buy-link.md`
- UI: `/toko/cart, /toko/:slug`
