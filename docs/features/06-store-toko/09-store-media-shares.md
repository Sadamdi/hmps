...
Fitur **Store Media Shares** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Store Media Shares** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
| DELETE | `/api/store/admin/shares/:shareId` | `server/routes/store.ts#L1642` | params: shareId | 500, 200/json |
...
- Feature doc: `06-store-toko/09-store-media-shares.md`
- UI: `/dashboard/toko`
