...
Fitur **Orders Invoices** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Orders Invoices** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
| Item | Value |
|------|-------|
| UI routes/surfaces | `/toko/orders, /toko/order/:orderNo, /dashboard/toko` |
...
|--------|----------|--------|----------------|-------------------|
| GET | `/api/store/admin/orders` | `server/routes/store.ts#L1653` | none observed in handler window | 500, 200/json |
| PATCH | `/api/store/admin/orders/:orderNo` | `server/routes/store.ts#L1666` | params: orderNo | 400, 404, 500, 200/json |
| DELETE | `/api/store/admin/orders/:orderNo` | `server/routes/store.ts#L1688` | params: orderNo | 400, 404, 500, 200/json |
| DELETE | `/api/store/admin/orders` | `server/routes/store.ts#L1702` | query: confirm | 400, 500, 200/json |
| GET | `/api/store/my-orders` | `server/routes/store.ts#L2059` | none observed in handler window | 500, 200/json |
| GET | `/api/store/orders/:orderNo` | `server/routes/store.ts#L2077` | params: orderNo; query: inv | 400, 404, 500, 200/json |
...
- Feature doc: `06-store-toko/06-orders-invoices.md`
- UI: `/toko/orders, /toko/order/:orderNo, /dashboard/toko`
