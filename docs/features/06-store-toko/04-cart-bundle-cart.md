...
Fitur **Cart Bundle Cart** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Cart Bundle Cart** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
|--------|----------|--------|----------------|-------------------|
| GET | `/api/store/cart` | `server/routes/store.ts#L1719` | none observed in handler window | 500, 200/json |
| POST | `/api/store/cart/items` | `server/routes/store.ts#L1803` | body: productId, qty | 400, 500, 200/json |
| POST | `/api/store/cart/bundles` | `server/routes/store.ts#L1874` | body: bundleId, qty | 400, 500, 200/json |
| PATCH | `/api/store/cart/items/:productId` | `server/routes/store.ts#L1934` | params: productId; body: qty | 404, 400, 500, 200/json |
| DELETE | `/api/store/cart/items/:productId` | `server/routes/store.ts#L1972` | params: productId | 500, 200/json |
| PATCH | `/api/store/cart/bundles/:bundleId` | `server/routes/store.ts#L1987` | params: bundleId; body: qty | 404, 400, 500, 200/json |
| DELETE | `/api/store/cart/bundles/:bundleId` | `server/routes/store.ts#L2022` | params: bundleId | 500, 200/json |
| POST | `/api/store/cart/draft` | `server/routes/store.ts#L2037` | none observed in handler window | 500, 200/json |

---

## Observed Request Shape

- `POST /api/store/cart/items` body fields observed: `productId, qty`
- `POST /api/store/cart/bundles` body fields observed: `bundleId, qty`
- `PATCH /api/store/cart/items/:productId` body fields observed: `qty`
- `PATCH /api/store/cart/bundles/:bundleId` body fields observed: `qty`
...
- Status JSON yang terdeteksi: `200/json, 400, 404, 500`
...
- Feature doc: `06-store-toko/04-cart-bundle-cart.md`
- UI: `/toko/cart`
