...
**Status**: Active | **Contract Confidence**: Verified from route scan  | **Category**: store toko

---

## Deskripsi

Fitur **Admin Store Operations** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Admin Store Operations** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
| Item | Value |
|------|-------|
| UI routes/surfaces | `/dashboard/toko` |
...
|--------|----------|--------|----------------|-------------------|
| POST | `/api/admin/migrate-community-media` | `server/routes.ts#L6757` | none observed in handler window | 403, 500, 200/json |
| POST | `/api/admin/permissions/recompute-owner` | `server/routes.ts#L8070` | none observed in handler window | 500, 200/json |
| GET | `/api/store/admin/access-summary` | `server/routes/store.ts#L789` | none observed in handler window | 500, 200/json |
| GET | `/api/store/admin/settings` | `server/routes/store.ts#L809` | none observed in handler window | 500, 200/json |
| GET | `/api/store/admin/categories` | `server/routes/store.ts#L819` | none observed in handler window | 500, 200/json |
| POST | `/api/store/admin/categories` | `server/routes/store.ts#L830` | body: name, slug, order | 400, 201, 500 |
| PATCH | `/api/store/admin/categories/:id` | `server/routes/store.ts#L857` | params: id | 404, 400, 500, 200/json |
| DELETE | `/api/store/admin/categories/:id` | `server/routes/store.ts#L891` | params: id | 404, 500, 200/json |
| POST | `/api/store/admin/upload-product-image` | `server/routes/store.ts#L906` | none observed in handler window | 400, 500, 200/json |
| POST | `/api/store/admin/uploads/cleanup` | `server/routes/store.ts#L923` | body: urls | 500, 200/json |
| PUT | `/api/store/admin/settings` | `server/routes/store.ts#L946` | none observed in handler window | 500, 200/json |
| GET | `/api/store/admin/campaigns` | `server/routes/store.ts#L1014` | none observed in handler window | 500, 200/json |
| POST | `/api/store/admin/campaigns` | `server/routes/store.ts#L1024` | none observed in handler window | 400, 201, 500 |
| PATCH | `/api/store/admin/campaigns/:id` | `server/routes/store.ts#L1066` | params: id | 404, 500, 200/json |
| DELETE | `/api/store/admin/campaigns/:id` | `server/routes/store.ts#L1100` | params: id | 404, 500, 200/json |
| GET | `/api/store/admin/bundles` | `server/routes/store.ts#L1113` | none observed in handler window | 500, 200/json |
| GET | `/api/store/admin/bundles/:id` | `server/routes/store.ts#L1126` | params: id | 404, 500, 200/json |
| POST | `/api/store/admin/bundles` | `server/routes/store.ts#L1137` | none observed in handler window | 400, 201, 500 |

---

## Observed Request Shape

- `POST /api/store/admin/categories` body fields observed: `name, slug, order`
- `POST /api/store/admin/uploads/cleanup` body fields observed: `urls`
- `PATCH /api/store/admin/products/reorder` body fields observed: `orderedIds`
- `POST /api/store/admin/products/:id/shares` body fields observed: `username, accessLevel`
...
- Feature doc: `06-store-toko/08-admin-store-operations.md`
- UI: `/dashboard/toko`
- Endpoint sources: `server/routes.ts`, `server/routes/store.ts`
