...
Fitur **Shipping Regional** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Shipping Regional** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
| Item | Value |
|------|-------|
| UI routes/surfaces | `checkout address/shipping form` |
...
| POST | `/api/store/shipping/quote` | `server/routes/store.ts#L609` | none observed in handler window | 400, 500, 200/json |
...
- Feature doc: `06-store-toko/07-shipping-regional.md`
- UI: `checkout address/shipping form`
