...
Fitur **Products Categories** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Products Categories** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
| Item | Value |
|------|-------|
| UI routes/surfaces | `/toko, /toko/:slug, /dashboard/toko` |
...
| GET | `/api/store/admin/products` | `server/routes/store.ts#L1255` | query: forReorder, page, limit | 500, 200/json |
| PATCH | `/api/store/admin/products/reorder` | `server/routes/store.ts#L1301` | body: orderedIds | 400, 500, 200/json |
| GET | `/api/store/admin/products/:id` | `server/routes/store.ts#L1329` | params: id | 404, 403, 500, 200/json |
| POST | `/api/store/admin/products` | `server/routes/store.ts#L1344` | none observed in handler window | 400, 201, 500 |
| PATCH | `/api/store/admin/products/:id` | `server/routes/store.ts#L1433` | params: id | 404, 403, 400, 500, 200/json |
| DELETE | `/api/store/admin/products/:id` | `server/routes/store.ts#L1580` | params: id | 404, 500, 200/json |
| GET | `/api/store/admin/products/:id/shares` | `server/routes/store.ts#L1597` | params: id | 500, 200/json |
| POST | `/api/store/admin/products/:id/shares` | `server/routes/store.ts#L1610` | params: id; body: username, accessLevel | 404, 400, 201, 500 |
...
- Feature doc: `06-store-toko/02-products-categories.md`
- UI: `/toko, /toko/:slug, /dashboard/toko`
- Endpoint sources: `server/routes/store.ts`
