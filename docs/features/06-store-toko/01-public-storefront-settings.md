...
Fitur **Public Storefront Settings** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Public Storefront Settings** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
| Item | Value |
|------|-------|
| UI routes/surfaces | `/toko, custom store path, /dashboard/toko` |
...
- Feature doc: `06-store-toko/01-public-storefront-settings.md`
- UI: `/toko, custom store path, /dashboard/toko`
