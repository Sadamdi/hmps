...
Fitur **Sharing Workflow** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Sharing Workflow** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
| Item | Value |
|------|-------|
| UI routes/surfaces | `shared content management` |
...
|--------|----------|--------|----------------|-------------------|
| POST | `/api/gdrive/check-access` | `server/routes.ts#L810` | body: url | 400, 200/json |
| POST | `/api/auth/forgot-password/request-otp` | `server/routes.ts#L1585` | body: email | 400, 429, 500, 200/json |
| POST | `/api/auth/change-password/request-otp` | `server/routes.ts#L1694` | none observed in handler window | 401, 404, 429, 500, 200/json |
| POST | `/api/auth/change-email/request-otp` | `server/routes.ts#L1907` | none observed in handler window | 401, 404, 429, 500, 200/json |
| POST | `/api/backups/restore/request-otp` | `server/routes.ts#L6072` | none observed in handler window | 400, 500, 200/json |
| POST | `/api/community/request-delete-otp` | `server/routes.ts#L9636` | none observed in handler window | 404, 429, 200/json |
| POST | `/api/registration/communities/:id/request-delete-otp` | `server/routes.ts#L9866` | params: id | 404, 400, 429, 200/json |
| GET | `/api/store/admin/access-summary` | `server/routes/store.ts#L789` | none observed in handler window | 500, 200/json |
| POST | `/manage/:id/decision` | `server/routes/feedback.ts#L613` | params: id; body: status, comment | 400, 404, 500, 200/json |
| POST | `/:entityType/:entityId/invite` | `server/routes/sharing.ts#L208` | params: entityType, entityId; body: targetUserId, permission | 400, 403, 404, 409, 201, 200/json |
| POST | `/:entityType/:entityId/request` | `server/routes/sharing.ts#L417` | params: entityType, entityId; body: permission | 409, 404, 201, 500, 200/json |
| POST | `/decision/:sharingId` | `server/routes/sharing.ts#L605` | params: sharingId; body: decision | 404, 500, 200/json |
| DELETE | `/:entityType/:entityId/access/:userId` | `server/routes/sharing.ts#L765` | params: entityType, entityId, userId: revokeUserId | 403, 404, 500, 200/json |
| GET | `/requestable` | `server/routes/sharing.ts#L1169` | query: entityType, q, limit | 400, 500, 200/json |

---

## Observed Request Shape

- `POST /api/gdrive/check-access` body fields observed: `url`
- `POST /api/auth/forgot-password/request-otp` body fields observed: `email`
- `POST /manage/:id/decision` body fields observed: `status`
- `POST /:entityType/:entityId/invite` body fields observed: `targetUserId`
- `POST /:entityType/:entityId/request` body fields observed: `permission`
- `POST /decision/:sharingId` body fields observed: `decision`
...
- Status JSON yang terdeteksi: `200/json, 201, 400, 401, 403, 404, 409, 429, 500`
...
- Feature doc: `08-collaboration-feedback/05-sharing-workflow.md`
- UI: `shared content management`
- Endpoint sources: `server/routes.ts`, `server/routes/feedback.ts`, `server/routes/sharing.ts`, `server/routes/store.ts`
