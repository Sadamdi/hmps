...
Fitur **Feedback Moderation Config** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Feedback Moderation Config** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
| Item | Value |
|------|-------|
| UI routes/surfaces | `/dashboard/feedback` |
...
| GET | `/api/library/manage` | `server/routes.ts#L3717` | none observed in handler window | 403, 500, 200/json |
| GET | `/api/prodi/manage` | `server/routes.ts#L5733` | none observed in handler window | 500, 200/json |
| PUT | `/api/prodi/manage` | `server/routes.ts#L5779` | body: curriculumYear, ...rest | 500, 200/json |
| GET | `/manage` | `server/routes/comments.ts#L341` | query: targetType, targetId | 500, 200/json |
| GET | `/config` | `server/routes/feedback.ts#L60` | none observed in handler window | 500, 200/json |
| GET | `/manage` | `server/routes/feedback.ts#L436` | query: target, type, hasReply, page, limit | 500, 200/json |
| GET | `/manage/counts-by-target` | `server/routes/feedback.ts#L462` | none observed in handler window | 500, 200/json |
| GET | `/manage/ratings` | `server/routes/feedback.ts#L483` | none observed in handler window | 500, 200/json |
| PATCH | `/manage/:id/visibility` | `server/routes/feedback.ts#L498` | params: id; body: visible | 400, 404, 500, 200/json |
| POST | `/manage/:id/reply` | `server/routes/feedback.ts#L518` | params: id; body: message | 400, 404, 500, 200/json |
| POST | `/manage/:id/decision` | `server/routes/feedback.ts#L613` | params: id; body: status, comment | 400, 404, 500, 200/json |
| GET | `/manage/config` | `server/routes/feedback.ts#L666` | none observed in handler window | 500, 200/json |
| PATCH | `/manage/config` | `server/routes/feedback.ts#L681` | body: destinations | 400, 200/json |
| PATCH | `/manage/:id` | `server/routes/feedback.ts#L792` | params: id; body: body, target, type | 404, 500, 200/json |
| DELETE | `/manage/:id` | `server/routes/feedback.ts#L814` | params: id | 500, 200/json |
| PATCH | `/manage/footer-display` | `server/routes/feedback.ts#L832` | none observed in handler window | 400, 500, 200/json |

---

## Observed Request Shape

- `PUT /api/prodi/manage` body fields observed: `curriculumYear`
- `PATCH /manage/:id/visibility` body fields observed: `visible`
- `POST /manage/:id/reply` body fields observed: `message`
- `POST /manage/:id/decision` body fields observed: `status`
- `PATCH /manage/config` body fields observed: `destinations`
- `PATCH /manage/:id` body fields observed: `body`
...
- Status JSON yang terdeteksi: `200/json, 400, 403, 404, 500`
