...
**Status**: Active | **Contract Confidence**: Verified from route scan  | **Category**: organization prodi

---

## Deskripsi

Fitur **Organization Periods Positions** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Organization Periods Positions** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
| Item | Value |
|------|-------|
| UI routes/surfaces | `/kelembagaan, /dashboard/kelembagaan` |
...
|--------|----------|--------|----------------|-------------------|
| GET | `/api/organization/periods` | `server/routes.ts#L4589` | none observed in handler window | 500, 200/json |
| POST | `/api/organization/periods` | `server/routes.ts#L4599` | body: period | 400, 500 |
| DELETE | `/api/organization/periods/:period` | `server/routes.ts#L4636` | params: period | 500, 200/json |
| GET | `/api/organization/positions/:period` | `server/routes.ts#L4674` | params: period | 500, 200/json |
| GET | `/api/organization/positions` | `server/routes.ts#L4685` | none observed in handler window | 500, 200/json |
| POST | `/api/organization/positions` | `server/routes.ts#L4695` | body: period, positions | 400, 201, 500 |
| POST | `/api/organization/positions/copy` | `server/routes.ts#L4733` | body: sourcePeriod, targetPeriod | 201, 500 |
| DELETE | `/api/organization/positions/:period` | `server/routes.ts#L4820` | params: period | 500, 204 |

---

## Observed Request Shape

- `POST /api/organization/periods` body fields observed: `period`
- `POST /api/organization/positions` body fields observed: `period`
- `POST /api/organization/positions/copy` body fields observed: `sourcePeriod`
...
- Status JSON yang terdeteksi: `200/json, 201, 204, 400, 500`
...
- Feature doc: `04-organization-prodi/01-organization-periods-positions.md`
- UI: `/kelembagaan, /dashboard/kelembagaan`
