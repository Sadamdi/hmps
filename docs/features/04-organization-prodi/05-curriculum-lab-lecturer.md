...
Fitur **Curriculum Lab Lecturer** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Curriculum Lab Lecturer** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
| Item | Value |
|------|-------|
| UI routes/surfaces | `/prodi/dosen/:slug, /prodi/curriculum/:slug, /prodi/laboratorium/:type/:index` |
...
|--------|----------|--------|----------------|-------------------|
| GET | `/api/prodi/curriculum/:year` | `server/routes.ts#L5757` | params: year | 400, 500, 200/json |
| POST | `/api/prodi/curriculum/year` | `server/routes.ts#L5806` | body: academicYear, copyFromYear | 500, 200/json |

---

## Observed Request Shape

- `POST /api/prodi/curriculum/year` body fields observed: `academicYear`
...
- Feature doc: `04-organization-prodi/05-curriculum-lab-lecturer.md`
- UI: `/prodi/dosen/:slug, /prodi/curriculum/:slug, /prodi/laboratorium/:type/:index`
