...
Fitur **Structure Copy Autofill** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Structure Copy Autofill** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
| Item | Value |
|------|-------|
| UI routes/surfaces | `/dashboard/kelembagaan` |
...
| POST | `/api/organization/structure/copy` | `server/routes.ts#L4753` | body: sourcePeriod, targetPeriod, overwrite | 400, 409, 201, 500 |
| POST | `/api/organization/structure-auto-fill` | `server/routes.ts#L4916` | body: period, members, positions | 400, 500, 200/json |
| POST | `/api/organization/structure-auto-fill/apply` | `server/routes.ts#L4989` | body: previewData, answers | 400, 500, 200/json |
...
- `POST /api/organization/structure/copy` body fields observed: `sourcePeriod`
- `POST /api/organization/structure-auto-fill` body fields observed: `period, members, positions`
- `POST /api/organization/structure-auto-fill/apply` body fields observed: `previewData, answers`
...
- Status JSON yang terdeteksi: `200/json, 201, 400, 409, 500`
...
- Feature doc: `04-organization-prodi/03-structure-copy-autofill.md`
- UI: `/dashboard/kelembagaan`
