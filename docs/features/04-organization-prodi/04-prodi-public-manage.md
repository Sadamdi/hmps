...
Fitur **Prodi Public Manage** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Prodi Public Manage** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
| Item | Value |
|------|-------|
| UI routes/surfaces | `/prodi, /dashboard/prodi` |
...
|--------|----------|--------|----------------|-------------------|
| GET | `/api/prodi` | `server/routes.ts#L5653` | none observed in handler window | 500, 200/json |
| GET | `/api/prodi/preview` | `server/routes.ts#L5663` | none observed in handler window | 400, 500 |
...
- Feature doc: `04-organization-prodi/04-prodi-public-manage.md`
- UI: `/prodi, /dashboard/prodi`
