...
Fitur **Organization Members** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Organization Members** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
|--------|----------|--------|----------------|-------------------|
| GET | `/api/organization/members` | `server/routes.ts#L4837` | query: period | 500, 200/json |
| GET | `/api/organization/members/:id` | `server/routes.ts#L4897` | params: id | 500, 200/json |
| POST | `/api/organization/members` | `server/routes.ts#L5023` | body: name, position, period, gdriveUrl | 400, 201 |
| PUT | `/api/organization/members/:id` | `server/routes.ts#L5121` | params: id; body: name, position, period, gdriveUrl | 400, 500, 200/json |
| DELETE | `/api/organization/members/:id` | `server/routes.ts#L5262` | params: id | 500, 200/json |

---

## Observed Request Shape

- `POST /api/organization/members` body fields observed: `name, gdriveUrl`
- `PUT /api/organization/members/:id` body fields observed: `name, gdriveUrl`
...
- Status JSON yang terdeteksi: `200/json, 201, 400, 500`
...
- Feature doc: `04-organization-prodi/02-organization-members.md`
