...
Fitur **Prodi Sync Media** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Prodi Sync Media** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
| Item | Value |
|------|-------|
| UI routes/surfaces | `/dashboard/prodi` |
...
|--------|----------|--------|----------------|-------------------|
| POST | `/api/prodi/upload/photo/member` | `server/routes.ts#L5874` | body: slug, oldPhotoUrl, profileUrl | 500, 200/json |
| POST | `/api/prodi/upload/photo/lab` | `server/routes.ts#L5912` | body: type, labIndex, imgIndex, oldPhotoUrl | 400, 500, 200/json |
| POST | `/api/prodi/upload/photo/org-structure` | `server/routes.ts#L5953` | body: oldPhotoUrl | 500, 200/json |
| POST | `/api/prodi/sync/run` | `server/routes.ts#L5979` | body: scope, overwrite | 409, 400, 200, 500, 200/json |

---

## Observed Request Shape

- `POST /api/prodi/upload/photo/member` body fields observed: `slug, oldPhotoUrl, profileUrl`
- `POST /api/prodi/upload/photo/lab` body fields observed: `type, labIndex, imgIndex, oldPhotoUrl`
- `POST /api/prodi/upload/photo/org-structure` body fields observed: `oldPhotoUrl`
- `POST /api/prodi/sync/run` body fields observed: `scope, overwrite`
...
- Status JSON yang terdeteksi: `200, 200/json, 400, 409, 500`
...
- Feature doc: `04-organization-prodi/06-prodi-sync-media.md`
- UI: `/dashboard/prodi`
