...
Fitur **Registration Community Admin** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Registration Community Admin** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
| Item | Value |
|------|-------|
| UI routes/surfaces | `/dashboard/registration` |
...
|--------|----------|--------|----------------|-------------------|
| GET | `/api/registration/communities` | `server/routes.ts#L9482` | none observed in handler window | 200/json |
| PUT | `/api/registration/communities/:id` | `server/routes.ts#L9501` | params: id; body: name, description, ownerUsername, ownerEmail, status | 404, 200/json |
| DELETE | `/api/registration/communities/:id` | `server/routes.ts#L9533` | params: id; body: challengeId, resetToken | 400, 404, 503, 200/json |
| POST | `/api/registration/communities/:id/request-delete-otp` | `server/routes.ts#L9866` | params: id | 404, 400, 429, 200/json |
| POST | `/api/registration/communities/:id/verify-delete-otp` | `server/routes.ts#L9905` | body: challengeId, otp | 400, 200/json |
| POST | `/api/registration/communities/:id/repair` | `server/routes.ts#L10445` | params: id; body: newPassword | 404, 200/json |
| GET | `/api/registration/communities/health` | `server/routes.ts#L10518` | none observed in handler window | 500, 200/json |

---

## Observed Request Shape

- `PUT /api/registration/communities/:id` body fields observed: `name`
- `DELETE /api/registration/communities/:id` body fields observed: `challengeId`
- `POST /api/registration/communities/:id/verify-delete-otp` body fields observed: `challengeId`
- `POST /api/registration/communities/:id/repair` body fields observed: `newPassword`
...
- Status JSON yang terdeteksi: `200/json, 400, 404, 429, 500, 503`
...
- Feature doc: `05-community-tenant/04-registration-community-admin.md`
- UI: `/dashboard/registration`
