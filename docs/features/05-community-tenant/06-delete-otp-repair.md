...
Fitur **Delete Otp Repair** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Delete Otp Repair** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
| Item | Value |
|------|-------|
| UI routes/surfaces | `community/admin lifecycle` |
...
| POST | `/api/community/verify-delete-otp` | `server/routes.ts#L9691` | body: challengeId, otp | 400, 200/json |
| DELETE | `/api/community` | `server/routes.ts#L9729` | body: challengeId, resetToken | 400, 404, 503, 200/json |
...
- `POST /api/community/verify-delete-otp` body fields observed: `challengeId`
- `DELETE /api/community` body fields observed: `challengeId`
...
- Status JSON yang terdeteksi: `200/json, 400, 404, 429, 503`
...
- Feature doc: `05-community-tenant/06-delete-otp-repair.md`
- UI: `community/admin lifecycle`
