...
Fitur **Registration Codes** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Registration Codes** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
|--------|----------|--------|----------------|-------------------|
| GET | `/api/registration/codes` | `server/routes.ts#L9320` | none observed in handler window | 200/json |
| POST | `/api/registration/codes` | `server/routes.ts#L9338` | body: type, maxUses, expiresInHours, note | 201 |
| DELETE | `/api/registration/codes/:id/permanent` | `server/routes.ts#L9377` | params: id; query: confirmUsedDelete; body: confirmUsedDelete | 404, 400, 200/json |
| PATCH | `/api/registration/codes/:id` | `server/routes.ts#L9409` | params: id; body: code, regenerateCode, maxUsesIncrement, extendHours, note | 404, 200/json |
| DELETE | `/api/registration/codes/:id` | `server/routes.ts#L9456` | params: id | 404, 400, 200/json |

---

## Observed Request Shape

- `POST /api/registration/codes` body fields observed: `type`
- `DELETE /api/registration/codes/:id/permanent` body fields observed: `confirmUsedDelete`
- `PATCH /api/registration/codes/:id` body fields observed: `code`
...
- Status JSON yang terdeteksi: `200/json, 201, 400, 404`
...
- Feature doc: `05-community-tenant/02-registration-codes.md`
