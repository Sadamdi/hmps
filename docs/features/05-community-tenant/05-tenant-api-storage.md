...
**Status**: Active | **Contract Confidence**: Partial from code/service scan  | **Category**: community tenant

---

## Deskripsi

Fitur **Tenant Api Storage** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Tenant Api Storage** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
- Feature doc: `05-community-tenant/05-tenant-api-storage.md`
- UI: `/:slug/*`
