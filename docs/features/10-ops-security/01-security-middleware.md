...
Fitur **Security Middleware** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Security Middleware** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
| Item | Value |
|------|-------|
| UI routes/surfaces | `all API routes` |
...
|--------|----------|--------|----------------|-------------------|
| GET | `/api/test/protection` | `server/routes.ts#L7303` | none observed in handler window | 200/json |
...
- Status JSON yang terdeteksi: `200/json`
...
- Feature doc: `10-ops-security/01-security-middleware.md`
- UI: `all API routes`
