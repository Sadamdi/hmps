...
Fitur **Cleanup Migration** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Cleanup Migration** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
- Status JSON yang terdeteksi: `200/json, 403, 410, 500`
...
- Feature doc: `10-ops-security/04-cleanup-migration.md`
