...
**Status**: Active | **Contract Confidence**: Partial from code/service scan  | **Category**: ops security

---

## Deskripsi

Fitur **Deployment Scheduler** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Deployment Scheduler** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
| Item | Value |
|------|-------|
| UI routes/surfaces | `server/index.ts, package scripts, nginx` |
...
- Feature doc: `10-ops-security/06-deployment-scheduler.md`
- UI: `server/index.ts, package scripts, nginx`
