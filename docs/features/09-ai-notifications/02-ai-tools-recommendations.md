...
**Status**: Active | **Contract Confidence**: Partial from code/service scan  | **Category**: ai notifications

---

## Deskripsi

Fitur **Ai Tools Recommendations** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Ai Tools Recommendations** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
| Item | Value |
|------|-------|
| UI routes/surfaces | `server/services/ai-tools.ts, recommendation.ts` |
...
- Feature doc: `09-ai-notifications/02-ai-tools-recommendations.md`
- UI: `server/services/ai-tools.ts, recommendation.ts`
