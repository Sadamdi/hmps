...
**Status**: Active | **Contract Confidence**: Verified from route scan  | **Category**: ai notifications

---

## Deskripsi

Fitur **Preferences Webpush** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Preferences Webpush** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
| Item | Value |
|------|-------|
| UI routes/surfaces | `notification prompt/preferences` |
...
|--------|----------|--------|----------------|-------------------|
| GET | `/preferences` | `server/routes/notifications.ts#L22` | none observed in handler window | 500, 200/json |
| PATCH | `/preferences` | `server/routes/notifications.ts#L42` | body: news, event, commentReply, feedbackReply, bugReply | 500, 200/json |
| POST | `/webpush/subscribe` | `server/routes/notifications.ts#L82` | body: endpoint, keys, preferences, guestSecret | 500, 200/json |
| DELETE | `/webpush/unsubscribe` | `server/routes/notifications.ts#L158` | body: endpoint | 400, 500, 200/json |
| GET | `/webpush/vapid-key` | `server/routes/notifications.ts#L179` | none observed in handler window | 200/json |
| GET | `/webpush/subscription-status` | `server/routes/notifications.ts#L185` | query: endpoint | 400, 500, 200/json |

---

## Observed Request Shape

- `PATCH /preferences` body fields observed: `news`
- `POST /webpush/subscribe` body fields observed: `endpoint`
- `DELETE /webpush/unsubscribe` body fields observed: `endpoint`
...
- Status JSON yang terdeteksi: `200/json, 400, 500`
...
- Feature doc: `09-ai-notifications/04-preferences-webpush.md`
- UI: `notification prompt/preferences`
- Endpoint sources: `server/routes/notifications.ts`
