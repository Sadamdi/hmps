...
Fitur **Notification Stream** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Notification Stream** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
| Item | Value |
|------|-------|
| UI routes/surfaces | `global notification stream` |
...
|--------|----------|--------|----------------|-------------------|
| GET | `/stream` | `server/routes/notifications.ts#L217` | query: guestSecret | 200 |
| GET | `/stream/stats` | `server/routes/notifications.ts#L281` | none observed in handler window | 200/json |
...
- Status JSON yang terdeteksi: `200, 200/json`
...
- Feature doc: `09-ai-notifications/03-notification-stream.md`
- UI: `global notification stream`
