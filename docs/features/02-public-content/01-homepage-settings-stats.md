...
Fitur **Homepage Settings Stats** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Homepage Settings Stats** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
| Item | Value |
|------|-------|
| UI routes/surfaces | `/, /dashboard, /dashboard/settings` |
...
|--------|----------|--------|----------------|-------------------|
| GET | `/api/settings` | `server/routes.ts#L5314` | none observed in handler window | 500 |
| PUT | `/api/settings` | `server/routes.ts#L5341` | none observed in handler window | 500, 200/json |
| PUT | `/api/settings/home-config` | `server/routes.ts#L5434` | body: blocks, navbar, navbarGroups, showDashboardLink | 500, 200/json |
| POST | `/api/settings/reset` | `server/routes.ts#L5555` | none observed in handler window | 500, 200/json |
| PUT | `/api/settings/home-image-slots` | `server/routes.ts#L5571` | body: slots | 400, 500, 200/json |
...
| GET | `/api/stats` | `server/routes.ts#L7076` | none observed in handler window | 500, 200/json |
| GET | `/api/dashboard/stats` | `server/routes.ts#L7096` | none observed in handler window | 403, 500, 200/json |
| GET | `/api/dashboard/activities` | `server/routes.ts#L7131` | query: limit, type | 200/json |
| POST | `/api/dashboard/log-activity` | `server/routes.ts#L7154` | none observed in handler window | 200/json |

---

## Observed Request Shape

- `PUT /api/settings/home-config` body fields observed: `blocks`
- `PUT /api/settings/home-image-slots` body fields observed: `slots`
...
- Status JSON yang terdeteksi: `200/json, 400, 403, 500`
...
- Feature doc: `02-public-content/01-homepage-settings-stats.md`
- UI: `/, /dashboard, /dashboard/settings`
