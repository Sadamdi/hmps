...
Fitur **Profil Page** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Profil Page** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
| Item | Value |
|------|-------|
| UI routes/surfaces | `/profil, /dashboard/profil` |
...
| POST | `/api/upload/filosofi` | `server/routes.ts#L7181` | body: key | 400, 500, 200/json |
...
- `POST /api/upload/filosofi` body fields observed: `key`
...
- Feature doc: `02-public-content/03-profil-page.md`
- UI: `/profil, /dashboard/profil`
