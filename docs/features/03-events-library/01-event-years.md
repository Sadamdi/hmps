...
Fitur **Event Years** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Event Years** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
|--------|----------|--------|----------------|-------------------|
| GET | `/api/event-years` | `server/routes.ts#L8107` | none observed in handler window | 500, 200/json |
| POST | `/api/event-years` | `server/routes.ts#L8117` | body: year | 400, 201, 409, 500 |
| PATCH | `/api/event-years/:id` | `server/routes.ts#L8142` | params: id | 404, 500, 200/json |
| PATCH | `/api/event-years/:id/activate` | `server/routes.ts#L8160` | params: id | 404, 500, 200/json |
| PATCH | `/api/event-years/:id/deactivate` | `server/routes.ts#L8187` | params: id | 404, 500, 200/json |
| GET | `/api/event-years/:id/events-count` | `server/routes.ts#L8205` | params: id | 500, 200/json |
| DELETE | `/api/event-years/:id` | `server/routes.ts#L8221` | params: id | 500, 200/json |

---

## Observed Request Shape

- `POST /api/event-years` body fields observed: `year`
...
- Status JSON yang terdeteksi: `200/json, 201, 400, 404, 409, 500`
...
- Feature doc: `03-events-library/01-event-years.md`
