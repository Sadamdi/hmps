...
Fitur **Library Public Cms** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Library Public Cms** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
| Item | Value |
|------|-------|
| UI routes/surfaces | `/library, /dashboard/library` |
...
|--------|----------|--------|----------------|-------------------|
| GET | `/api/library` | `server/routes.ts#L3666` | none observed in handler window | 500, 200/json |
| GET | `/api/library/manage` | `server/routes.ts#L3717` | none observed in handler window | 403, 500, 200/json |
| GET | `/api/library/slug/:slug` | `server/routes.ts#L3831` | params: slug | 400, 404, 500, 200/json |
| GET | `/api/library/:id` | `server/routes.ts#L3849` | params: id | 404, 500, 200/json |
| GET | `/api/library/:libraryId/folder/:folderId/files` | `server/routes.ts#L3904` | params: libraryId, folderId | 404, 503, 500, 200/json |
| POST | `/api/library` | `server/routes.ts#L3960` | none observed in handler window | 400, 401, 201, 500 |
| PUT | `/api/library/:id` | `server/routes.ts#L4223` | params: id | 404, 400, 500, 200/json |
| DELETE | `/api/library/:id` | `server/routes.ts#L4548` | params: id | 400, 404, 500, 200/json |
...
- Status JSON yang terdeteksi: `200/json, 201, 400, 401, 403, 404, 500, 503`
...
- Feature doc: `03-events-library/04-library-public-cms.md`
- UI: `/library, /dashboard/library`
