...
Fitur **Events Public Cms** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Events Public Cms** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
| Item | Value |
|------|-------|
| UI routes/surfaces | `/events/*, /dashboard/events` |
...
|--------|----------|--------|----------------|-------------------|
| GET | `/api/events/published` | `server/routes.ts#L8256` | none observed in handler window | 500, 200/json |
| GET | `/api/events/active-home` | `server/routes.ts#L8266` | none observed in handler window | 500, 200/json |
| GET | `/api/events/by-year/:year` | `server/routes.ts#L8297` | params: year; query: parentOnly | 400, 404, 500, 200/json |
| GET | `/api/events` | `server/routes.ts#L8311` | query: yearId, parentId | 400, 403, 500, 200/json |
| GET | `/api/events/year/:year/slug/:slug` | `server/routes.ts#L8478` | params: year, slug; query: children | 400, 404, 403, 500, 200/json |
| GET | `/api/events/:id` | `server/routes.ts#L8550` | params: id; query: children | 404, 403, 500, 200/json |
| POST | `/api/events` | `server/routes.ts#L8604` | body: parentId | 400, 201, 500 |
| PATCH | `/api/events/:id` | `server/routes.ts#L8811` | params: id | 404, 403, 400, 500, 200/json |
| DELETE | `/api/events/:id` | `server/routes.ts#L9063` | params: id | 404, 403, 500, 200/json |
| POST | `/api/events/:id/copy-to-berita` | `server/routes.ts#L9112` | params: id; body: copyAttachments | 404, 403, 201 |
| POST | `/api/events/:id/attach-berita` | `server/routes.ts#L9174` | params: id; body: beritaId, copyFiles | 404, 403, 400, 200/json |
| DELETE | `/api/events/:id/attach-berita/:beritaId` | `server/routes.ts#L9210` | params: id, beritaId | 404, 403, 200/json |

---

## Observed Request Shape

- `POST /api/events` body fields observed: `parentId`
- `POST /api/events/:id/copy-to-berita` body fields observed: `copyAttachments`
- `POST /api/events/:id/attach-berita` body fields observed: `beritaId`
...
- Feature doc: `03-events-library/02-events-public-cms.md`
- UI: `/events/*, /dashboard/events`
