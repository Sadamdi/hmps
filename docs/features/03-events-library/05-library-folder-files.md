...
Fitur **Library Folder Files** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Library Folder Files** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
- Status JSON yang terdeteksi: `200/json, 404, 500, 503`
...
- Feature doc: `03-events-library/05-library-folder-files.md`
- UI: `/library/:id`
