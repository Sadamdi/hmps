...
Fitur **Otp Password Email** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Otp Password Email** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
| Item | Value |
|------|-------|
| UI routes/surfaces | `/forgot-password, profile security` |
...
|--------|----------|--------|----------------|-------------------|
| POST | `/api/auth/change-password` | `server/routes.ts#L1538` | body: currentPassword, newPassword | 401, 404, 500, 200/json |
| POST | `/api/auth/forgot-password/request-otp` | `server/routes.ts#L1585` | body: email | 400, 429, 500, 200/json |
| POST | `/api/auth/forgot-password/verify-otp` | `server/routes.ts#L1624` | body: challengeId, otpCode | 400, 500, 200/json |
| POST | `/api/auth/forgot-password/confirm` | `server/routes.ts#L1654` | body: challengeId, resetToken, newPassword | 400, 404, 500, 200/json |
...
| POST | `/api/auth/change-password/confirm` | `server/routes.ts#L1732` | body: challengeId, otpCode, currentPassword, newPassword | 400, 401, 404, 500, 200/json |
| POST | `/api/auth/change-email/request-otp` | `server/routes.ts#L1907` | none observed in handler window | 401, 404, 429, 500, 200/json |
| POST | `/api/auth/change-email/confirm` | `server/routes.ts#L1948` | body: challengeId, otpCode, newEmail | 400, 401, 500, 200/json |

---

## Observed Request Shape

- `POST /api/auth/change-password` body fields observed: `currentPassword`
- `POST /api/auth/forgot-password/request-otp` body fields observed: `email`
- `POST /api/auth/forgot-password/verify-otp` body fields observed: `challengeId`
- `POST /api/auth/forgot-password/confirm` body fields observed: `challengeId`
- `POST /api/auth/change-password/confirm` body fields observed: `challengeId`
- `POST /api/auth/change-email/confirm` body fields observed: `challengeId`
...
- Status JSON yang terdeteksi: `200/json, 400, 401, 404, 429, 500`
...
- Feature doc: `01-auth-access/03-otp-password-email.md`
- UI: `/forgot-password, profile security`
