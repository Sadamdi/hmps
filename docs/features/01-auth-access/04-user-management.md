...
Fitur **User Management** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **User Management** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
| Item | Value |
|------|-------|
| UI routes/surfaces | `/dashboard/users` |
...
|--------|----------|--------|----------------|-------------------|
| POST | `/api/users/:id/password` | `server/routes.ts#L1790` | params: id; body: newPassword | 404, 400, 403, 500, 200/json |
| POST | `/api/users/:id/email` | `server/routes.ts#L1996` | params: id; body: newEmail | 400, 404, 403, 500, 200/json |
| PUT | `/api/users/:id/role` | `server/routes.ts#L2067` | params: id; body: role, division | 400, 404, 500, 200/json |
| GET | `/api/users` | `server/routes.ts#L2111` | none observed in handler window | 500, 200/json |
| POST | `/api/users` | `server/routes.ts#L2164` | body: username, password, name, email, role, division, divisionLabel, | 400, 201, 500 |
| PUT | `/api/users/:id` | `server/routes.ts#L2232` | params: id; body: username, name, email, role, division, divisionLabel, password, | 400, 404, 403, 500, 200/json |
| DELETE | `/api/users/:id` | `server/routes.ts#L2333` | params: id | 400, 404, 403, 500, 200/json |
| PUT | `/api/users/:id/role` | `server/routes.ts#L7897` | params: id; body: role | 400, 404, 500, 200/json |
| GET | `/api/users/:id/permission-overrides` | `server/routes.ts#L7934` | params: id | 404, 403, 500, 200/json |
| PUT | `/api/users/:id/permission-overrides` | `server/routes.ts#L7970` | params: id; body: allow, deny | 404, 403, 400, 500, 200/json |

---

## Observed Request Shape

- `POST /api/users/:id/password` body fields observed: `newPassword`
- `POST /api/users/:id/email` body fields observed: `newEmail`
- `PUT /api/users/:id/role` body fields observed: `role`
- `POST /api/users` body fields observed: `username`
- `PUT /api/users/:id` body fields observed: `username`
- `PUT /api/users/:id/role` body fields observed: `role`
- `PUT /api/users/:id/permission-overrides` body fields observed: `allow`
...
- Feature doc: `01-auth-access/04-user-management.md`
- UI: `/dashboard/users`
