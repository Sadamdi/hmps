...
Fitur **Session Management** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Session Management** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
| Item | Value |
|------|-------|
| UI routes/surfaces | `account/session surfaces` |
...
|--------|----------|--------|----------------|-------------------|
| POST | `/api/auth/revoke-all-sessions` | `server/routes.ts#L1369` | none observed in handler window | 401, 500, 200/json |
| GET | `/api/auth/sessions` | `server/routes.ts#L1474` | none observed in handler window | 500, 200/json |
| POST | `/api/auth/sessions/revoke` | `server/routes.ts#L1510` | body: sessionId | 400, 404, 500, 200/json |

---

## Observed Request Shape

- `POST /api/auth/sessions/revoke` body fields observed: `sessionId`
...
- Status JSON yang terdeteksi: `200/json, 400, 401, 404, 500`
...
- Feature doc: `01-auth-access/02-session-management.md`
- UI: `account/session surfaces`
