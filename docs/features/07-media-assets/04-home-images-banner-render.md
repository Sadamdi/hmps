...
**Status**: Active | **Contract Confidence**: Verified from route scan  | **Category**: media assets

---

## Deskripsi

Fitur **Home Images Banner Render** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Home Images Banner Render** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
|--------|----------|--------|----------------|-------------------|
| GET | `/api/home-images/active` | `server/routes.ts#L6203` | none observed in handler window | 500 |
| GET | `/api/home-images` | `server/routes.ts#L6233` | none observed in handler window | 500, 200/json |
| POST | `/api/home-images` | `server/routes.ts#L6254` | body: year | 400, 201, 500 |
| PUT | `/api/home-images/:year` | `server/routes.ts#L6285` | params: year | 404, 500, 200/json |
| DELETE | `/api/home-images/:year` | `server/routes.ts#L6303` | params: year | 404, 500, 200/json |
| POST | `/api/home-images/:year/set-active` | `server/routes.ts#L6328` | params: year | 404, 500, 200/json |
| POST | `/api/home-images/:year/copy` | `server/routes.ts#L6346` | params: year; body: targetYear, overwrite | 400, 200/json |
| POST | `/api/home-images/:year/banner-render` | `server/routes.ts#L6373` | params: year; body: personName, divisionText, themeColor, showDivisionName, showLogo | 404, 500 |
| POST | `/api/home-images/:year/upload/:slot` | `server/routes.ts#L6440` | params: year, slot | 400, 404, 500, 200/json |
| DELETE | `/api/home-images/:year/slot/:slot` | `server/routes.ts#L6538` | params: year, slot | 400, 404, 500, 200/json |
| POST | `/api/home-images/:year/upload-person/:slot` | `server/routes.ts#L6601` | params: year, slot | 400, 404, 500, 200/json |
| DELETE | `/api/home-images/:year/person/:slot` | `server/routes.ts#L6694` | params: year, slot | 400, 404, 500, 200/json |

---

## Observed Request Shape

- `POST /api/home-images` body fields observed: `year`
- `POST /api/home-images/:year/copy` body fields observed: `targetYear`
- `POST /api/home-images/:year/banner-render` body fields observed: `personName, divisionText, themeColor, showDivisionName, showLogo`
...
- Status JSON yang terdeteksi: `200/json, 201, 400, 404, 500`
...
- Feature doc: `07-media-assets/04-home-images-banner-render.md`
