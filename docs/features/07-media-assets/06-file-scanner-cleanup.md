...
Fitur **File Scanner Cleanup** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **File Scanner Cleanup** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
| Item | Value |
|------|-------|
| UI routes/surfaces | `ops/admin cleanup` |
...
|--------|----------|--------|----------------|-------------------|
| POST | `/api/assets/cleanup-orphans` | `server/routes.ts#L6181` | none observed in handler window | 410 |
...
- Status JSON yang terdeteksi: `200/json, 410, 500`
...
- Feature doc: `07-media-assets/06-file-scanner-cleanup.md`
- UI: `ops/admin cleanup`
