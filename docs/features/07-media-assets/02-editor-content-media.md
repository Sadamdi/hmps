...
Fitur **Editor Content Media** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Editor Content Media** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
| Item | Value |
|------|-------|
| UI routes/surfaces | `dashboard editors` |
...
|--------|----------|--------|----------------|-------------------|
| POST | `/api/upload/content-image` | `server/routes.ts#L2385` | body: beritaId | 400, 500, 200/json |
| POST | `/api/upload/berita-attachment` | `server/routes.ts#L2419` | body: beritaId | 400, 500, 200/json |
| POST | `/api/upload/event-content-image` | `server/routes.ts#L2446` | body: eventId, parentEventId | 400, 500, 200/json |
...
- `POST /api/upload/content-image` body fields observed: `beritaId`
- `POST /api/upload/berita-attachment` body fields observed: `beritaId`
- `POST /api/upload/event-content-image` body fields observed: `eventId, parentEventId`
...
- Feature doc: `07-media-assets/02-editor-content-media.md`
