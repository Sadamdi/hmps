...
Fitur **General Upload** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **General Upload** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
| Item | Value |
|------|-------|
| UI routes/surfaces | `dashboard upload forms` |
...
|--------|----------|--------|----------------|-------------------|
| POST | `/api/upload` | `server/routes.ts#L7212` | body: oldFileUrl, category | 400, 500, 200/json |

---

## Observed Request Shape

- `POST /api/upload` body fields observed: `oldFileUrl, category`
...
- Feature doc: `07-media-assets/01-general-upload.md`
- UI: `dashboard upload forms`
