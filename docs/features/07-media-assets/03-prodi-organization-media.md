...
Fitur **Prodi Organization Media** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Prodi Organization Media** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
- Feature doc: `07-media-assets/03-prodi-organization-media.md`
- UI: `/dashboard/prodi`
