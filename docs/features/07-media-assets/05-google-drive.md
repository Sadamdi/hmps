...
Fitur **Google Drive** terdokumentasi ulang dari audit code HMPS New, bukan dari payload template. Endpoint, parameter, body field, dan status response di bawah berasal dari static scan terhadap route handler di `server/routes.ts` dan `server/routes/*.ts` dan hanya membaca file HMPS New.

---

## User Stories

1. Sebagai user/admin HMPS, saya ingin memakai fitur **Google Drive** melalui UI terkait agar kebutuhan operasional atau informasi terpenuhi.
...
| Item | Value |
|------|-------|
| UI routes/surfaces | `library/store/media flows` |
...
| POST | `/api/gdrive/media-url` | `server/routes.ts#L859` | body: fileId, url, mediaType: userSpecifiedType | 400, 500, 200/json |
| POST | `/api/gdrive/folder-contents` | `server/routes.ts#L7248` | body: folderId, url | 400, 500, 200/json |
...
- `POST /api/gdrive/media-url` body fields observed: `fileId`
- `POST /api/gdrive/folder-contents` body fields observed: `folderId`
...
- Feature doc: `07-media-assets/05-google-drive.md`
- UI: `library/store/media flows`
